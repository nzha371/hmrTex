...
When we think of a stream we often picture water flowing through a fixed channel or pipe. Likewise in computer programming, it refers to the flow of data. A stream is basically a sequence of data bits made available over time. It can be thought of a channel connecting a processor (where the data is processed according to given set of instructions) to the input and output device.

An input stream is when the stream acts as a source stream for the program where the data is transferred to be processed from an input device to the program. The program requests the data transfer through a read operation known as input stream/standard input.

An output stream is when the stream acts as a destination stream for the output device where data is transferred from the program to the output device. The program transfers its output data through a write operation known as output stream/standard output.

Standard error is another output stream used by the program to output error messages or diagnostics. This stream is independent of standard output and is useful for identifying errors produced by the program. While it is independent, it is acceptable to direct the standard output and the standard error to the same output destination such as a text terminal.

\subsubsection{Example}

We can look at a basic example to understand how streaming works.

\includegraphics[width=14cm]{streamex}\\

The keyboard which is our input device writes data to the stdin (standard input) stream which is then processed by the computer program. Output is written to the stdout (standard output) stream and any errors is written to the stderr (standard error) stream shown on the display which is our output device. When we call print in the program, we are really just writing to the stdout variable. 

\section{Hadoop Streaming}

Hadoop streaming makes use of the same streaming process where we discussed earlier. It is a utility that comes with the Hadoop distribution which allow users to create and run MapReduce jobs to be executed in the Hadoop cluster using other languages. By default the Hadoop MapReduce framework is written in Java but it utilizes Unix streams to pipe between Hadoop and our MapReduce program so we can use any language which can read standard input and write to standard output for our MapReduce program. MapReduce programs can be written in multiple languages such as R, Python, Perl, PHP, C++, etc. The utility is packed in a JAR file. 
...
\section{Concept of Formatting}

Formatting is a requirement in streaming, to enable programs to interpret the data we parse to it. Instead of reading data from a physical input device such as keyboard, Hadoop streaming is designed to handle and parse large volumes of data which can be completely unstructured as we discussed earlier in Chapter 1: Concepts of Big Data. Parsing the unstructured data to the program is achieved through the input streams and is generally not an issue, however for the data to be processed by our MapReduce programs, we need to give it a structure. This is what we refer to as the formatting. The formatter takes each row of our input data and turns it into into something that the program can compute on. The raw bytes from our input stream are initially parsed as chunks consisting of single row values. The formatter defines and separates the values for every record of data in the chunk.The next step is for the formatter to identify the class/data type for each of the values. Once we have the structure, the data in each chunk can be interpreted by our programs, which in our case is the mapper and the reducer. 
...
$HADOOP_HOME/bin/hadoop jar $HADOOP_HOME/hadoop-streaming.jar 
    ...
    -output myOutputDir 
    -mapper /bin/wc -l 
    -reducer /usr/bin/awk '{n=n+$1}END{print n}' 
\end{verbatim}
...
In the example above, myInputDirs is the input directory for the mapper and myOutputDir is the output directory for the reducer. The mapper executes the word count command in linux and the reducer aggregates the word count as a total.  

\section{Implementation of Hadoop Streaming}

Let us now explore how Hadoop streaming works.\\

\includegraphics[width=14cm]{hadoopstreaming}\\

\subsection{Input Format}

The first part of the Hadoop streaming process involves the Input Format which defines the input specifications for the MapReduce Job. The Input Format is the class which selects the file from HDFS that should be input to the map function. It is responsible for the split on the input data so that the data can be correctly assigned to the mappers. The data is divided into blocks (typically 64/128mb) in HDFS which is the assigned to the mappers for further processing. We know that the Inputs and Outputs for the map and reduce functions are key-value pairs separated by a tab character. Hence the Input Format is also responsible for converting the data into the key-value pairs for the mapper. This is called the RecordReader. Another benefit of using Hadoop streaming is that the user can specify their own formatter for the input and output using another language. 

\subsection{Mapper \& Reducer}

We will be mainly focusing on this stage in our paper. Mappers and Reducers are the Hadoop servers that run the Map and Reduce programs respectively. 

Both the map and reduce function are specified by the user which can also written in another language made available through Hadoop streaming. The Map function takes the data from the standard input as values, processes them, and produces another set of intermediate key-value pairs as standard output. The Reduce function takes standard inputs as key-value pairs, and produces key-value pairs as standard output.

In the Map stage, each block is assigned to a Mapper which runs in parallel. The number of mappers is decided by the Hadoop framework. it based on the size of the data and the memory block available on each mapper server. After the mappers complete processing, the framework shuffles and sorts the results before passing the data onto the reducers. It is important to note that a reducer cannot start while a mapper is still in progress. All the map output values that have the same key are assigned to a single reducer to process. The reducer then aggregates the values for each specific key and returns the output as key-value pairs.
...
It is highly efficient by parses data in binary and utilises chunk-wise processing as well as automated conversion to and from R objects. This enables a smooth and efficient transition between R and Hadoop. There are also other packages available in R which uses the traditional key-value operations such as the \pkg{rmr2} package.
...
\section{Formatting and Streaming in R}

The HMR package is essentially a Hadoop streaming API which runs in R. This allows the user to run MapReduce jobs and write formatter functions in R as opposed to writing scripts in the native language Java. \\

The idea is stream the data from a file path/connection in Hadoop and parse into R as individual blocks represented in raw binary format. We apply the formatter to the rows of each chunk. The formatter defines the structure of the data for each columns of values along with its class/data type. This structure is a function which is applied to every chunk of data produced from the split. The result contains chunks of data converted into R objects in the form of a matrix or data frame. Each chunk contains rows of values separated by a specified delimiter.\\

Each chunk will be associated with each map process. The mapper takes in the data frame of values as standard input and executes the user defined mapper function on it. The  mapper function identifies the keys from the values associated with it. Hence, the output generated by the mapper process is a structured object containing the key and its values associated with it separated by a tab character. If the reducer process has not been specified by the user, the output will contain the key-value pairs across each row. It is important to note that each key will be unique to each chunk. In other words, we will not find multiple keys in the same chunk. \\

The chunks of data frames containing of key-value pairs (known as the intermediate values) are not visible if the reduce process has been specified by the user. We again apply the formatter to the intermediate values from the mapper and define the structure. Each chunk of intermediate values are associated with each reduce process. The reducer takes in the data frame of key-value pairs as standard input and executes the user defined reducer function on it. The output of the reducer is also structured key-value pairs. The values associated with each key is aggregated since each key is unique to each chunk, however the number of chunks in our output is dependent on how many reducers the user chose to specify. \\

Lastly, the chunks are converted back into the raw binary format and parsed back into Hadoop. This is stored in HDFS which can be accessed through Hadoop. 
