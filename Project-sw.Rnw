...
When we think of a stream we often picture water flowing through a fixed channel or pipe. Likewise in computer programming, it refers to the flow of data. A stream is basically a sequence of data bits made available over time. The stream comes from various sources, in varying speed and volumes and flow into a single, continuous combined stream. We look at streaming from the program's perspective which is a logical connection between the program and the file/source. 

The program can get input from a data source by reading in data from a stream attached to a source. Once the data is processed by the program, it produces output by writing the data to an output stream attached to a destination. 
...
\includegraphics[width=14cm]{streamingex}\\
...
\section{Concept of Formatting - Specifiying Input Data}

In the example above, we added specifications on how the data is read into R. As we know it is a csv file, logically we should the fields in with comma separators. What if our data file is not structured in a way we can easily read, or what if the file is too big for us to read efficiently. We have to create a specific function which tells us how we want to structure our input data after it is parsed into our program. For the data to be computable by the program, we need to create the specifications of the data, in other words the metadata which is the data that provides information about our data file. This includes information like how the fields should be separated, how many fields we have, the classes/ data types of our fields, etc.
...
The output is saved to a directory in HDFS and can be access with the unix command hadoop fs -ls /folder name and printed using hadoop fs -cat /folder name. \\

\includegraphics[width=14cm]{hmrresult}\\
